...
class DefectReportView extends GetView<DefectReportController> {
              ...
              if (history.isEmpty) {
                return const Card(
                  child: Padding(
                    ...
                    child:
                        Text('No previous defects reported for this shipment.'),
                  ),
                );
              }
                    ...
                    child: Container(
                      ...
                      child: const Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 8),
                          Text('Tap to Capture / Upload Image',
  ...
  Widget _buildAnalysisResultCard(Map<String, dynamic> data) {
    ...
    final blockchainTxHash = data['blockchain_tx_hash']?.toString() ?? '';
      ...
      child: Column(
        ...
        children: [
          ...
          _buildResultRow('Condition',
              isClear ? 'Clear: Normal' : 'Defect: $formattedDefectType'),
          _buildResultRow('Confidence', '$confidence%'),
          _buildResultRow(
              'Shipment ID', data['shipment_id']?.toString() ?? '-'),
          ...
          if (data['damage_location'] != null &&
            ...
            _buildResultRow(
                'Damage Location', data['damage_location'].toString()),
          _buildResultRow(
              'Explanation', data['explanation']?.toString() ?? '-'),
          const Divider(height: 24),
          const Text('Blockchain Evidence Data',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
          const SizedBox(height: 8),
          _buildResultRow('Status', blockchainStatus),
          if (blockchainTxHash.isNotEmpty && blockchainTxHash != 'null')
            _buildHashRow('Tx Hash', blockchainTxHash),
          _buildHashRow(
              'Shipment Hash', data['shipment_hash']?.toString() ?? '-'),
          _buildHashRow(
              'Evidence Hash', data['evidence_hash']?.toString() ?? '-'),
          _buildHashRow('Image Hash', data['image_hash']?.toString() ?? '-'),
          _buildResultRow(
              'Chain ID', data['defect_type_chain_id']?.toString() ?? '-'),
          _buildResultRow(
              'Confidence BPS', data['confidence_bps']?.toString() ?? '-'),
        ],
      ),
    );
  }
